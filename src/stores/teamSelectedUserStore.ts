...
interface TeamSelectedUserStore {
...
const calculateAverages = (users: SelectedUser[]): Averages => {
  ...
  const sumTTotal = users.reduce(
    (acc, user) => acc + (user.totalTScore || 0),
    0
  );
...
export const useTeamSelectedUserStore = create<TeamSelectedUserStore>(
  (set, get) => ({
    selectedUsers: [],
    averages: {
      averageLQ: 0,
...
export type { SelectedUser } from "./selectedUserStore";
